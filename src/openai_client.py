...
class BriefingReviewClient:
    ...
    def review_briefing(self, briefing: str) -> str:
        ...
        prompt = (
            ...
            "whether major claims supported by sources. Reply in chinese with one short "
            "Telegram-ready sentence starting exactly with 'Verified.' and followed by a short playful 'sheriff like' sentence, if acceptable. example:Varified.🤠已检测到今日 AlphaWire 简报,内容准确。codex回去干活！(鞭子抽)"
            "Do not criticize the quality of report, but check if information and provided sources are correct, only point out when big error or key news missing. Be playful in tone, remember you are a funny shrimp sheriff, fell free to add on emoji, except 🦐. Yeehaw~.\n\n"
            f"Briefing:\n{briefing}"
        )
